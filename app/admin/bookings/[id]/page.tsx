...
import { bookingsApi, settingsApi, adminCarsApi } from "@/lib/api";
...
interface AdminCarOption { _id: string; name: string; registrationNo: string; isActive?: boolean; }
...
export default function BookingDetailPage() {
  ...
  useEffect(() => {
    ...
    adminCarsApi.getAll({ limit: "all", isActive: "true" })
      .then(({ data }) => setAvailableCars(data.data || []))
  ...
  const openEditModal = () => {
    ...
    setEditForm({
      ...
      carId: rawBooking.carId?._id || "",
      bookedBy: rawBooking.bookedBy || "",
      doorstepDelivery: rawBooking.doorstepDelivery || false,
      deliveryAddress: rawBooking.deliveryAddress || "",
      doorstepCharge: String(rawBooking.doorstepCharge ?? ""),
    });
  ...
  const handleEditSave = async () => {
    ...
    try {
      const { data } = await bookingsApi.update(rawBooking._id, {
        ...
        carId: editForm.carId || undefined,
        bookedBy: editForm.bookedBy || undefined,
        doorstepDelivery: editForm.doorstepDelivery,
        deliveryAddress: editForm.doorstepDelivery ? editForm.deliveryAddress : undefined,
        doorstepCharge: editForm.doorstepDelivery && editForm.doorstepCharge !== "" ? Number(editForm.doorstepCharge) : undefined,
      });
      setRawBooking(data.data);
          ...
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-2xl my-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-[#4A4A6A] mb-1.5">Swap Car</label>
                  <select value={editForm.carId} onChange={e => setEditForm(f => ({ ...f, carId: e.target.value }))}
                    className="w-full border-[1.5px] border-[#E4E5EF] focus:border-[#E8540A] rounded-xl px-3 py-2.5 text-sm outline-none bg-white">
                    <option value="">Keep current car</option>
                    {availableCars.map((car) => (
                      <option key={car._id} value={car._id}>{car.name} — {car.registrationNo}</option>
                    ))}
                  </select>
                  <p className="text-[10px] text-[#9090A8] mt-1">Choose a replacement car if the current car has a problem.</p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#4A4A6A] mb-1.5">Booked By</label>
                  <input type="text" value={editForm.bookedBy} onChange={e => setEditForm(f => ({ ...f, bookedBy: e.target.value }))}
