...
export default function ClosingBillsPage() {
  ...
  useEffect(() => { setPage(1); load(1); }, [dateFrom, dateTo]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const totals = rows.reduce(
    (acc, r) => {
      ...
      if (amt > 0) acc.due += amt;
      else if (amt < 0 && !r.closingBill?.refundPaid) acc.refund += Math.abs(amt);
      return acc;
    },
    { due: 0, refund: 0 }
  );
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[#9090A8] text-sm">{total} closed bookings · final settlement bills</p>
        </div>
