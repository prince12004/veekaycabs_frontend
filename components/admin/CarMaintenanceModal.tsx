...
import { Plus, X, Loader2, IndianRupee, Trash2, CheckCircle2, XCircle, Car } from "lucide-react";
...
export default function CarMaintenanceModal({
  carId,
  onClose,
  onChanged,
}: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        {toast && (
          <div className={cn(
            "fixed top-5 right-5 z-[60] flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium text-white transition-all",
        ...
        <div className="p-5 border-b border-[#E4E5EF] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#FFF3ED] flex items-center justify-center shrink-0">
              <Car size={18} className="text-[#E8540A]" />
            </div>
            <div>
              <h2 className="font-black text-[#0F0F1A] text-base font-syne">{car?.name || "Loading..."}</h2>
              <p className="text-[#9090A8] text-xs font-mono">
                {car?.registrationNo}{car?.odometer ? ` · ${car.odometer.toLocaleString("en-IN")} km` : ""}
              </p>
            </div>
          </div>
          <button onClick={onClose} className="text-[#9090A8] hover:text-[#EF4444] transition-colors">
            <X size={20} />
          </button>
        </div>

        <div className="p-5 space-y-4 overflow-y-auto">
          {/* Filters + Add Expense trigger */}
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-[#E8540A] text-white rounded-xl text-sm font-semibold hover:bg-[#c94508] transition-colors shrink-0"
            >
              <Plus size={15} className={cn("transition-transform", formOpen && "rotate-45")} /> {formOpen ? "Cancel" : "Add Expense"}
            </button>
          </div>

          {/* Inline Add Expense form */}
          {formOpen && (
            <div className="bg-[#FAFAFA] border border-[#E4E5EF] rounded-2xl p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#4A4A6A] mb-1.5">Category</label>
                  <select value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))} className="w-full border border-[#E4E5EF] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#E8540A] bg-white">
                    {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={form.remark} onChange={(e) => setForm((f) => ({ ...f, remark: e.target.value }))} rows={2} className="w-full border border-[#E4E5EF] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#E8540A] resize-none" placeholder="e.g. Front brake pads replaced" />
              </div>
              <button
                onClick={handleAdd}
                disabled={saving}
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-[#E8540A] text-white rounded-xl text-sm font-bold hover:bg-[#c94508] transition-colors disabled:opacity-60"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white rounded-2xl border border-[#E4E5EF] p-3.5">
              <div className="flex items-center gap-1.5 text-[#9090A8] text-[11px] font-semibold mb-1">
                <IndianRupee size={12} /> Total {allTime ? "(all time)" : "(month)"}
              </div>
              <p className="text-lg font-black text-[#0F0F1A]">Rs. {totalSpent.toLocaleString("en-IN")}</p>
              <p className="text-[#9090A8] text-[11px] mt-0.5">{logs.length} entries</p>
            </div>
            {byCategory.slice(0, 3).map((c) => (
              <div key={c.category} className="bg-white rounded-2xl border border-[#E4E5EF] p-3.5">
                <div className="flex items-center gap-1.5 mb-1">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: CATEGORY_COLOR[c.category] || "#9090A8" }} />
                  <span className="text-[#9090A8] text-[11px] font-semibold truncate">{c.category}</span>
                </div>
                <p className="text-lg font-black text-[#0F0F1A]">Rs. {c.total.toLocaleString("en-IN")}</p>
                <p className="text-[#9090A8] text-[11px] mt-0.5">{c.count} entries</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E4E5EF] overflow-hidden">
            {loading ? (
              <div className="p-8 flex justify-center"><Loader2 size={22} className="animate-spin text-[#E8540A]" /></div>
            ) : logs.length === 0 ? (
              <div className="p-8 text-center text-sm text-[#9090A8]">No maintenance entries found for this filter.</div>
            ) : (
              <div className="divide-y divide-[#F1F2F7]">
                {logs.map((log) => (
                  <div key={log._id} className="px-4 py-3.5 flex items-start justify-between gap-3 hover:bg-[#FAFAFA] transition-colors">
